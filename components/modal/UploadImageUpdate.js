...
function UploadImageModalUpdate({ open, onClick, fileId, id, section }) {
  ...
  useEffect(() => {
    console.log(fileId)
    fetchCategory();
    section === 'update' && open ? fetchDetail() : reset();
  }, [open]);
          ...
          <div className="mt-4 text-black">
            <div className="border h-[150px] w-[150px] mx-auto mb-5">
              {image ? (
                ...
                <label
                  className="flex justify-center items-center h-[100%]"
                  htmlFor={`file-${'image'.replace(/ /g, '-')}-update`}
                >
                  <MdFileUpload sx={{ marginRight: "10px" }} />
                  Image
                </label>
              )}
              <input type="file" hidden id={"file-image-update"} onChange={(e) => setImage(e.target.files[0])} />
            </div>
            <div className="mb-2">
              <InputSingleField
...
export default UploadImageModalUpdate;
